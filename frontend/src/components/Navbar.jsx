      ...
      <nav id="main-nav" className={`sticky top-0 w-full z-50 bg-[#FCF9F6]/80 backdrop-blur-xl border-b border-outline-variant/10 transition-all duration-500 ${isScrolled ? 'h-16 md:h-16 shadow-sm' : 'h-16 md:h-24'}`}>
        <div className="flex items-center max-w-custom mx-auto h-full px-6 md:px-12 relative">
          ...
          <div className="flex-1 flex justify-start">
            <Link to="/" className={`shrink-0 transition-all duration-500 ${isSearchExpanded ? 'opacity-100 md:opacity-100 md:scale-100' : 'opacity-100 scale-100'}`}>
              <img
          ...
          <div className={`flex items-center justify-end gap-3 md:gap-8 text-[#1c1c1a] transition-all duration-500 ${isSearchExpanded ? 'flex-6' : 'flex-1'}`}>
            {/* Expanding Search Bar */}
            <div className={`relative flex items-center transition-all duration-1000 ease-[cubic-bezier(0.19,1,0.22,1)] ${isSearchExpanded ? 'w-[55vw] sm:w-[65vw] md:w-full md:max-w-[1000px]' : 'w-10 md:w-12'}`}>
              <button
                onClick={() => setIsSearchExpanded(true)}
                className={`flex items-center justify-center hover:text-primary transition-all duration-500 z-10 ${isSearchExpanded ? 'absolute left-0 pointer-events-none text-primary scale-110' : 'w-10 h-10'}`}
              >
              ...
              <input
                ...
                className={`bg-transparent border-b border-on-surface/10 py-2 transition-all duration-1000 ease-[cubic-bezier(0.19,1,0.22,1)] font-jakarta-sans text-[10px] uppercase tracking-widest outline-none focus:border-primary/40 ${isSearchExpanded ? 'w-full pl-10 md:pl-12 pr-10 md:pr-12 opacity-100 shadow-none' : 'w-0 opacity-0 pointer-events-none border-none'}`}
              ...
              {isSearchExpanded && (
                <button
                  ...
                  className="absolute right-0 hover:text-primary transition-all duration-500 flex items-center justify-center text-outline/30 hover:rotate-180"
                >
            ...
            <div className="flex items-center gap-2 md:gap-8 transition-all duration-500 opacity-100 w-auto">
